...
# recent changes:
The `tag_banlist` configuration item has moved from `common.py` to `checkpoints.py`. Although these are not technically checkpoint related, it was decided to try and centralise all of the file related configuration in one file. The three main scripts were completely refactored, apologies in advance for any introduced breakage.
...
`--use_freeu`, `--freeu_b1`, `--freeu_b2`, `--freeu_s1`, `--freeu_s2` - applies [FreeU](https://github.com/ChenyangSi/FreeU) to the model via ComfyUI's built in support for FreeU V2.
...
_On inline lora support generally_: in-prompt lora handling is fiddly and low on my priorities to fix - I tend to instead create a new checkpoint tuple to handle these. While lora will be parsed out of most prompts, they do not work with the `--force_prompt` option on `image_remaker`, and you can't introduce a lora using the conditional form of `--mandatory_tags`.

`--rerun_lora` - if this is true, prompts for a given lora bucket will also be run in the `base` bucket (without any lora loaded). This can let you test the strength or effectiveness of a given lora.
...
`--banlist` - passed to the DTG llm (and the WD14 tagger when using `image_remaker`) to specify tags should not be included in generated prompts - useful if your tagger overdetects things like moles. By default, takes its value from a list in the `checkpoints.py` file.

`--add_banlist` - provide a second banlist to concatenate to the first one - useful if your default is 99% correct but you just want to add a handful of terms at a time.
...
`--shake` adds or subtracts weight from prompt tags randomly by adding `random.gauss(0, <shake>)` to each term. Terms are then rounded to two decimal places.
...
`--detailer_selector` - by default, the script carries out two different detailing pipelines (three if an `fd_checkpoint` is specified) 
- 0: the model object paired with the provided positive prompt, 
- 1: the model object paired with the checkpoint's default positive prompt
- 3: the fd_checkpoint model with its default positive prompt, if an fd_checkpoint is provided

Pipeline 0 is 'closest' to what will have been used to generate the original image, but you may find that very rich prompts lead to bad faces (as the model will attempt to diffuse in e.g. tagged background elements). By providing indexes, only those process will be executed. The default is -1, which means that all the pipelines will be run and saved. I tend to use 1.

`--steps`, `--denoise`, and `--cycles` - parameters for face detailing. The defaults are 20 steps, 0.35 denoise strength, and two cycles.
...
## pausing diffusion
`--diffusion_start` and `--diffusion_stop` - the program will pause from diffusing images outside of the time period specified by these arguments. For limiting power use during very large scale diffusion runs.
...
`--prompted_only` - Similarly to `image_from_text` which provides multiple default detailing pipelines, the program will attempt to detail with both the prompt provided and an 'empty prompt'. If you specify `--prompted_only` and provide a manual prompt, this empty prompt pipeline will be disabled.
